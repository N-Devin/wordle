...
function addLetter(letter) {
  ...
  highlightBox(state.currentRow, state.currentCol);
...
function removeLetter() {
  var prev_row = state.currentRow;
  var prev_col = state.currentCol - 1;
  if (state.currentCol === 0) return;
  state.grid[state.currentRow][state.currentCol - 1] = '';
  remove_highlightBox(prev_row, prev_col);

  var prev_row = state.currentRow;
  var prev_col = state.currentCol ;
  state.currentCol--;
}
...
function remove_highlightBox(row, col) {
  const box = document.getElementById(`box${row}${col}`);
  box.classList.add('highlight_rm');
}
